        ...
        messages: [
          {
            ...
            content: [
              ...
              'Input records include leadId and leadName. Every issue item must include a non-empty leadName copied from the matching input record.',
              ...
              'Return arrays of all material deterministicIssues up to 25 items per issue category; do not return only one representative issue when multiple issues exist.',
              'If deterministicIssues contains rows, the report must include those lead-specific rows in the matching issue arrays unless the row is clearly contradicted by records.',
              'Summaries, insights, and action plans must quantify deterministicIssueBreakdown counts and name the highest-impact leads when present.',
